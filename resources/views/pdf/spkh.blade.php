...
<head>
    ...
    <title>Surat Pengantar Keterangan Kehilangan</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="page">
        ...
        {{-- ===== JUDUL ===== --}}
        <div class="surat-title">Surat Pengantar Keterangan Kehilangan</div>
        <div class="surat-nomor">
            Nomor : {{ $permohonan->nomor_surat ?? '100.2/......-SMPL/.../.........../......' }}
        </div>
        ...
        $tglPengantar = isset($data['tanggal_surat_pengantar'])
            ...
            : '-';
        $tglKehilangan = isset($data['tanggal_kehilangan'])
        ...
        $barangLines = array_values(array_filter(array_map('trim', explode("\n", $data['barang_hilang'] ?? ''))));
        if (empty($barangLines) && !empty($data['barang_hilang'])) {
            $barangLines = [trim($data['barang_hilang'])];
        }
        @endphp
        ...
        {{-- ===== NARASI + DATA KEHILANGAN ===== --}}
        <p class="narasi" style="margin-top:6px;">
            Berdasarkan pernyataan yang bersangkutan dan surat pengantar Ketua RT. {{ $rtPad }}
            RW. {{ $rwPad }}
            Nomor: {{ $data['no_surat_pengantar'] ?? '....' }}
            tanggal {{ $tglPengantar }}, menerangkan bahwa :
        </p>
        ...
        <table class="data-table" style="margin-bottom:4px;">
            <tr>
                ...
                <td class="col-sep">:</td>
                <td class="col-value">{{ ($data['hari_kehilangan'] ?? '-') . ', ' . strtoupper($tglKehilangan) }}</td>
            </tr>
            <tr>
                <td class="col-label">Jam</td>
                <td class="col-sep">:</td>
                <td class="col-value">{{ isset($data['jam_kehilangan']) ? $data['jam_kehilangan'] . ' WITA' : '-' }}</td>
            </tr>
            <tr>
                <td class="col-label">Di</td>
                <td class="col-sep">:</td>
                <td class="col-value">{{ $data['lokasi_kehilangan'] ?? '-' }}</td>
            </tr>
        </table>

        {{-- DAFTAR BARANG HILANG --}}
        <p class="narasi">Nama tersebut diatas melaporkan telah kehilangan/tercecer antara lain :</p>
        @if(!empty($barangLines))
        <ol class="barang-list">
            @foreach($barangLines as $item)
            <li>{{ $item }}</li>
            @endforeach
        </ol>
        @else
        <p class="narasi" style="margin-left:16px;">1. .........</p>
        @endif

        <p class="penutup">
            Demikian surat pengantar keterangan kehilangan ini diberikan untuk dapat dpergunakan

<footer class="bg-white border-t border-gray-100 py-6">
    <div class="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <p class="text-sm text-gray-500">
            &copy; {{ date('Y') }} <span class="font-semibold text-gray-700">{{ auth()->user()?->kecamatan?->nama ?? auth()->user()?->kelurahan?->kecamatan?->nama ?? auth()->user()?->kabupaten?->nama ?? 'SiMPEL' }}</span>. All rights reserved.
        </p>

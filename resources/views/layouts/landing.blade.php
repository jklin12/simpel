    ...
    <header
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <nav class="hidden lg:flex items-center gap-8">
                    <a href="{{ route('portal.home') }}" class="text-base font-medium text-gray-700 hover:text-primary-600 transition-colors {{ request()->routeIs('portal.home') ? 'text-primary-600' : '' }}">Beranda</a>
                    <a href="{{ route('portal.berita') }}" class="text-base font-medium text-gray-700 hover:text-primary-600 transition-colors {{ request()->routeIs('portal.berita*') ? 'text-primary-600' : '' }}">Berita</a>
                    <a href="{{ route('portal.peta') }}" class="text-base font-medium text-gray-700 hover:text-primary-600 transition-colors {{ request()->routeIs('portal.peta*') ? 'text-primary-600' : '' }}">Peta Wilayah</a>
                    <a href="{{ route('portal.struktur-organisasi') }}" class="text-base font-medium text-gray-700 hover:text-primary-600 transition-colors {{ request()->routeIs('portal.struktur-organisasi*') ? 'text-primary-600' : '' }}">Struktur Organisasi</a>

                    {{-- Dropdown Layanan ditiadakan sementara agar rapi
                <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <button class="flex items-center gap-1 text-sm font-medium text-gray-600 hover:text-primary-600 transition-colors focus:outline-none">
                        Layanan
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute z-50 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" style="display: none;">
                        <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                            <a href="{{ route('services.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-primary-600" role="menuitem">Buat Surat</a>
                    <a href="{{ route('tracking.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-primary-600" role="menuitem">Cek Status</a>
            </div>
        </div>
        </div> --}}
        ...
        <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="lg:hidden bg-white border-t border-gray-100 shadow-lg absolute w-full z-40" style="display: none;">
            <div class="px-4 pt-2 pb-6 space-y-1">
                <a href="{{ route('portal.home') }}" class="block px-3 py-3 text-base font-medium text-gray-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg">Beranda</a>
                <a href="{{ route('portal.berita') }}" class="block px-3 py-3 text-base font-medium text-gray-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg">Berita</a>
                <a href="{{ route('portal.peta') }}" class="block px-3 py-3 text-base font-medium text-gray-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg">Peta Wilayah</a>
                <a href="{{ route('portal.struktur-organisasi') }}" class="block px-3 py-3 text-base font-medium text-gray-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg">Struktur Organisasi</a>
                <a href="#buat-surat" class="block px-3 py-3 text-base font-medium text-gray-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg">Layanan Surat</a>
                ...
                <div id="contact">
                    ...
                    <ul class="space-y-3">
                        <li class="flex items-start gap-3 text-sm text-gray-500">
                            ...
                            Jl. Kenanga, RT.06/RW.09, Kelurahan Landasan Ulin Timur, Kecamatan Landasan Ulin, Kota Banjarbaru, Kalimantan Selatan, kode pos 70724
                        </li>

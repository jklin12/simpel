            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8" x-data="skbHandler()">
                ...
                <script>
                    window.updateHariBerangkat = function(dateStr) {
                        const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
                        if (!dateStr) return;
                        const d = new Date(dateStr + 'T00:00:00');
                        const hariEl = document.getElementById('hari_berangkat');
                        if (hariEl) hariEl.value = days[d.getDay()];
                    };
                    document.addEventListener('DOMContentLoaded', function() {
                        const tglEl = document.getElementById('tanggal_berangkat');
                        if (tglEl && tglEl.value) window.updateHariBerangkat(tglEl.value);
                    });
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Tujuan <span class="text-red-500">*</span></label>
                        <input type="text" name="tujuan" value="{{ old('tujuan') }}" placeholder="Contoh: Jakarta" class="w-full rounded-lg border-gray-300 bg-gray-50 focus:bg-white focus:ring-primary-500 focus:border-primary-500 transition-colors py-3 px-4" required>
                        @error('tujuan') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Untuk Keperluan <span class="text-red-500">*</span></label>
                        <input type="text" name="keperluan" value="{{ old('keperluan') }}" placeholder="Contoh: Berobat / Mengurus Administrasi" class="w-full rounded-lg border-gray-300 bg-gray-50 focus:bg-white focus:ring-primary-500 focus:border-primary-500 transition-colors py-3 px-4" required>
                        @error('keperluan') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Berangkat <span class="text-red-500">*</span></label>
                            <input type="date" name="tanggal_berangkat" id="tanggal_berangkat" value="{{ old('tanggal_berangkat') }}"
                                onchange="updateHariBerangkat(this.value)"
                            ...
                            @error('tanggal_berangkat') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Hari <span class="text-red-500">*</span></label>
                            <input type="text" name="hari_berangkat" id="hari_berangkat" value="{{ old('hari_berangkat') }}" placeholder="Otomatis dari tanggal" class="w-full rounded-lg border-gray-300 bg-gray-50 focus:bg-white focus:ring-primary-500 focus:border-primary-500 transition-colors py-3 px-4" required readonly>
                            @error('hari_berangkat') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div x-data="{
                        ...
                        addItem() { if (this.items.length < 10) { this.items.push(''); } },
                        removeItem(i) { if (this.items.length > 1) this.items.splice(i, 1); },
                        get joinedValue() { return this.items.filter(v => v.trim() !== '').join('\n'); }
                    }">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Pengikut
                            <span class="ml-1 text-xs text-gray-500 font-normal">(opsional — kosongkan jika berangkat sendiri)</span>
                        </label>
                        <input type="hidden" name="pengikut" :value="joinedValue">
                        <div class="space-y-2">
                        ...
                        <button type="button" @click="addItem()" x-show="items.length < 10"
                        ...
                        @error('pengikut') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="file" name="skb_surat_pengantar_rtrw" accept=".jpg,.jpeg,.png,.pdf" class="w-full rounded-lg border border-gray-300 bg-gray-50 focus:ring-primary-500 focus:border-primary-500 transition-colors py-2 px-3 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100" required>
                        @error('skb_surat_pengantar_rtrw') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="skb_ktp_kk_pemohon" accept=".jpg,.jpeg,.png,.pdf" class="w-full rounded-lg border border-gray-300 bg-gray-50 focus:ring-primary-500 focus:border-primary-500 transition-colors py-2 px-3 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100" required>
                        @error('skb_ktp_kk_pemohon') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
